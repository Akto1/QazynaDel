...
print(f"DEBUG: Ищу папку static по пути: {static_dir}")

print(f"DEBUG: Папка существует? {os.path.exists(static_dir)}")
...
async def _auth(request) -> Optional[dict]:
    ...
    print(f"DEBUG: Пытаюсь авторизовать init_data: {init_data[:20]}...")  # Печатаем начало для проверки
    ...
    if not user:
        ...
        print("DEBUG: ОШИБКА АВТОРИЗАЦИИ: validate_init_data вернул None")
...
async def handle_categories(request):
    ...
    print("DEBUG: Получен запрос на /api/categories")
...
async def handle_category_products(request: web.Request) -> web.Response:
    ...
    try:
        ...
        print(f"DEBUG: match_info пришел такой: {request.match_info}")
        ...
        print(f"!!! ОШИБКА КЛЮЧА ИЛИ ЧИСЛА: {e}")
    ...
    # Добавь это временно прямо перед return web.json_response(out)
        ...
        print(f"DEBUG: Полный JSON для отправки: {out}")
...
def build_app(bot: Bot) -> web.Application:
    ...
    print("DEBUG: Роуты успешно зарегистрированы")
